    ...
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            ...
            if (!empty($title) && !empty($content)) {
                try {
                    $stmt = $pdo->prepare("
                        INSERT INTO noticeboard (title, content, priority, author_id, expiry_date)
                        VALUES (?, ?, ?, ?, ?)
                    ");
                    $stmt->execute([$title, $content, $priority, $_SESSION['user_id'], $expiry_date]);
            ...
            if (!empty($title) && !empty($content) && !empty($id)) {
                try {
                    $stmt = $pdo->prepare("
                        UPDATE noticeboard
                        SET title = ?, content = ?, priority = ?, expiry_date = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE id = ?
                    ");
                    $stmt->execute([$title, $content, $priority, $expiry_date, $is_active, $id]);
...
<head>
    ...
    <title>Manage Noticeboard - Admin Dashboard</title>
...
<body>
    ...
    <div class="admin-header">
        <div class="admin-nav">
            <div class="admin-brand">Noticeboard Management</div>
    ...
    <div class="dashboard-container">
        ...
        <div class="form-container">
            <h3><i class="fas fa-plus-circle"></i> Add New Notice</h3>
        ...
        <div class="notices-table">
            <div class="table-header">
                <h3 class="table-title"><i class="fas fa-list"></i> All Notices</h3>
            </div>
                        ...
                        <?php if (isset($notices) && !empty($notices)): ?>
                            <?php foreach ($notices as $notice): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="priority-badge priority-<?php echo isset($notice['priority']) ? $notice['priority'] : 'medium'; ?>">
                                            <?php echo ucfirst(isset($notice['priority']) ? $notice['priority'] : 'medium'); ?>
                                        </span>
                                    </td>
                                    <td><?php echo htmlspecialchars($notice['author_name']); ?></td>
                                    <td>
                                        <span class="status-badge <?php echo (isset($notice['is_active']) && $notice['is_active']) ? 'status-active' : 'status-inactive'; ?>">
                                            <?php echo (isset($notice['is_active']) && $notice['is_active']) ? 'Active' : 'Inactive'; ?>
                                        </span>
                                    </td>
                                    <td>
                                        ...
                                        <br>
                                        <small style="color: var(--text-light);">
                                            <?php echo date('g:i A', strtotime($notice['created_at'])); ?>
                                        </small>
                                    </td>
                                    <td class="actions-cell">
                                        <button class="btn btn-secondary" onclick="editNotice(<?php echo $notice['id']; ?>)">
                                            <i class="fas fa-edit"></i> Edit
                                        </button>
                                        <form method="POST" action="" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this notice?');">
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <textarea id="edit_content" name="content" class="form-textarea" rows="6" required></textarea>
                </div>

                <div class="form-group">
                    <label class="form-label" for="edit_expiry_date">Expiry Date (Optional)</label>
                    <input type="datetime-local" id="edit_expiry_date" name="expiry_date" class="form-input">
                </div>
    ...
    <script>
        function editNotice(noticeId) {
            // Find the notice data and populate the modal
            ...
            if (notice) {
                ...
                document.getElementById('edit_priority').value = notice.priority || 'medium';
                document.getElementById('edit_content').value = notice.content || '';
                document.getElementById('edit_expiry_date').value = notice.expiry_date ? notice.expiry_date.slice(0, 16) : '';
                document.getElementById('edit_is_active').checked = (notice.is_active == 1);
